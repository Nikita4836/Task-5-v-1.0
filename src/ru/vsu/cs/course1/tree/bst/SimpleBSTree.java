...
public class SimpleBSTree<T extends Comparable<? super T>> extends BinaryTree<T> implements DefaultBSTree<T> {
     ...
     */
    private T put(SimpleTreeNode<T> node, T value) {
        ...
        if (cmp == 0) {
            ...
            if (cmp < 0) {
                if (node.left == null) {
                    node.left = new SimpleTreeNode<T>(value);
                    size++;
                    return null;
                } else {
                ...
                if (node.right == null) {
                    node.right = new SimpleTreeNode<T>(value);
                    size++;
                    return null;
                } else {
     ...
     * @return Старое значение, равное value, если есть
     */
    private T remove(SimpleTreeNode<T> node, SimpleTreeNode<T> nodeParent, T value)
    {
        if (node == null) {
            return null;
        }
        ...
        if (cmp == 0) {
            ...
            if (node.left != null && node.right != null) {
                ...
                SimpleTreeNode<T> minParent = getMinNodeParent(node.right);
                if (minParent == null) {
                ...
                SimpleTreeNode<T> child = (node.left != null) ? node.left : node.right;
                if (nodeParent == null) {
    ...
    public double min(SimpleTreeNode node) {
        double min = (int)node.value;
        if (node.left != null) {
            if (min > min(node.left)){
                min = min(node.left);
            }
        }
        return min;
     ...
     */
    private SimpleTreeNode<T> getMinNodeParent(SimpleTreeNode<T> node) {
        if (node == null) {
            return null;
        }
    ...
    public T put(T value) {
        if (root == null) {
            root = new SimpleTreeNode<T>(value);
            size++;
            return null;
        }
